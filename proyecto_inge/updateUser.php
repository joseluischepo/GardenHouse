...
$name = $apellido = $correo = $numero = $contra = $tipo = $apikey = "";
$name_err = $apellido_err = $correo_err = $numero_err = $contra_err = $tipo_err = $apikey_err =  "";
...
if(isset($_POST["idus"]) && !empty($_POST["idus"])){
   ...
   //Validar celular 1 (cel1)
    ...
    // Validate apikey
    $input_apikey = trim($_POST["apikey"]);
    if(empty($input_apikey)){
        $apikey_err = "Por favor ingresa la apikey.";     
    } else{
        $apikey = $input_apikey;
    }
    ...
    if(empty($name_err) && empty($apellido_err) && empty($correo_err) && empty($numero_err) && empty($contra_err) && empty($tipo_err) && empty($apikey_err)){
        // Prepare an update statement
        $sql = "UPDATE usuarios SET name=?, apellido=?, correo=?, numero=?, contra=?, tipo=?, apikey=?  WHERE idus=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssi",$param_name,$param_apellido,$param_correo,$param_numero,$param_contra,$param_tipo,$param_apikey,$param_idus);
                            ...
                            <div class="card-body card-block" >
                                <form class="form-horizontal"action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row form-group <?php echo (!empty($apellido_err)) ? 'has-error' : ''; ?>">
                                        ...
                                        <div class="col-12 col-md-9"><input type="text"name="apellido" id="apellido" placeholder="Ingrese su primer apellido" value="<?php echo $apellido; ?>" class="form-control"><small class="form-text text-muted">Nombre</small></div>
                                    ...
                                    <div class="row form-group <?php echo (!empty($numero_err)) ? 'has-error' : ''; ?>">
                                        ...
                                        <div class="col-12 col-md-9"><input type="text" name="numero" id="numero" placeholder="9999999" value="<?php echo $numero; ?>" class="form-control"><small class="form-text text-muted">Nombre</small></div>
                                    ...
                                    <div class="row form-group <?php echo (!empty($apikey_err)) ? 'has-error' : ''; ?>">
                                        <div class="col col-md-3"><label for="text-input" class=" form-control-label">Apikey</label></div>
                                        <div class="col-12 col-md-9"><input type="text"name="apikey" id="apikey" placeholder="Ingrese su apikey" value="<?php echo $apikey; ?>" class="form-control"><small class="form-text text-muted">Apikey</small></div>
                                        <span class="help-block"><?php echo $apikey_err;?></span>
                                    </div>
