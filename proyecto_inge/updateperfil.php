...
if(isset($_POST["idus"]) && !empty($_POST["idus"])){
   ...
   //Validar celular 1 (cel1)
    ...
    if(empty($name_err) && empty($apellido_err) && empty($correo_err) && empty($numero_err) && empty($contra_err)){
        ...
        $sql = "UPDATE usuarios SET name=?, apellido=?, correo=?, numero=?, contra=? WHERE idus=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssi",$param_name,$param_apellido,$param_correo,$param_numero,$param_contra,$param_idus);
                    ...
                    <div class="col-lg-12">
                        <div class="card" >
                            <div class="card-header">
                                <strong>Perfil</strong> actualización
                            </div>
